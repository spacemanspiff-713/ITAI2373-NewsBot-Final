...
def article_intelligence() -> None:
    ...
    if st.button("Analyze article ✦", type="primary"):
        ...
        with evidence:
            ...
            st.dataframe(entities if not entities.empty else pd.DataFrame({"Notice": ["No supported entities were detected."]}), width="stretch", hide_index=True)
            if result.get("relationships"):
                st.caption("Relationship labels are transparent heuristics; co-occurrence is not a proven real-world relationship.")
                st.dataframe(pd.DataFrame(result["relationships"]), width="stretch", hide_index=True)
        with context:
            st.dataframe(pd.DataFrame(result.get("topics") or []), width="stretch", hide_index=True)
            neighbors = pd.DataFrame(result.get("semantic_neighbors") or [])
            if not neighbors.empty:
                st.markdown("### Related local coverage")
                st.dataframe(neighbors[[column for column in ["article_id", "title", "category", "score"] if column in neighbors]], width="stretch", hide_index=True)
        if result.get("warnings"):
            st.warning(" · ".join(result["warnings"]))
        st.download_button("Download analysis JSON", serialize_json(result), "newsbot_analysis.json", "application/json")
...
def batch_studio() -> None:
    ...
    if st.button("Run batch analysis"):
        ...
        try:
            ...
            st.dataframe(table.style.format({"Confidence": "{:.1%}"}), width="stretch", hide_index=True)
            st.download_button("Download batch JSON", serialize_json(results), "newsbot_batch.json", "application/json")
...
def visual_evidence() -> None:
    ...
    with st.expander("Browse the full visual library"):
        ...
        for index, path in enumerate(figures):
            columns[index % 3].image(str(path), caption=path.stem.replace("_", " ").title(), width="stretch")
...
def explorer() -> None:
    ...
    st.dataframe(shown[["date", "category", "title", "authors", "link"]].sort_values("date", ascending=False), width="stretch", hide_index=True)
